...
const vacuumAfterMigrationDirective = "-- migrate: vacuum-after";
...
for (const fileName of fs.readdirSync(migrationsDir).filter((name) => name.endsWith(".sql")).sort()) {
  ...
  apply();
  if (sql.includes(vacuumAfterMigrationDirective)) {
    shouldVacuum = true;
  }
...
if (shouldVacuum) {
  db.exec("VACUUM");
  logger.info("Vacuumed database after migration request");
}
