...
export {
  ...
  listLocationDepartureRows,
  listLocationDepartures,
  listServices,
  listServicesWithScheduledDepartures,
  listTimetableDocuments,
  londonLocalTimestampResponse
};
...
function listLocationDepartures(db: Database.Database, serviceId: number, queryDate: string): DepartureResponse[] {
  return listLocationDepartureRows(db, serviceId, queryDate).map(departureResponseFromRow);
}

function listServicesWithScheduledDepartures(db: Database.Database): Set<number> {
  const rows = db.prepare(`
...
function listServices(db: Database.Database): ServiceResponse[] {
  const now = new Date();
  const rows = db.prepare(`
...
function listInstallationServices(db: Database.Database, installationId: string): ServiceResponse[] {
  const rows = db.prepare(`
...
function listTimetableDocuments(db: Database.Database, serviceId?: number): TimetableDocumentResponse[] {
  return createTimetableDocumentResponses(db, serviceId);
}

function etagForJson(value: unknown): string {
